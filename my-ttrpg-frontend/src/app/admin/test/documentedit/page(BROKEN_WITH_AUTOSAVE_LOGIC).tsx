...
export default function DocumentEditPage() {
	...
	async function handleServerFileLoad(filename: string) {
		...
		try {
			...
			const res = await fetch(`/api/files/download/${encodeURIComponent(filename)}?t=${Date.now()}`, { credentials: 'include', cache: 'reload' });
	...
	async function saveDocumentToServer() {
		...
		try {
			...
			await fetch(`/api/files/delete/${encodeURIComponent(fileName.replace(/\.havdoc$/, '.havdoc_auto'))}`, {
				method: 'DELETE',
				credentials: 'include',
			});
